...
async function resolveBundledFilePath(key, loadBundledFile, fallbackPath, metaUrl, options = {}) {
  ...
  {
    const loadedModule = await loadBundledFile();
    const loadedPath2 = loadedModule?.default;
    if (loadedPath2 === undefined || loadedPath2 === null) {
      const fallbackPathResolved = resolveFallbackFilePath(fallbackPath, metaUrl);
      if (existsSync(fallbackPathResolved)) {
        return fallbackPathResolved;
      }
      console.warn(`OpenTUI-TERMUX-PATCH: bundled asset ${JSON.stringify(key)} has no default export, fallback missing at ${fallbackPathResolved}`);
    }
    return normalizeLoadedFilePath(loadedPath2, metaUrl);
  }
}
...
function normalizeLoadedFilePath(loadedPath, baseUrl) {
  if (!loadedPath) {
    throw new Error(`OpenTUI-TERMUX-PATCH: bundled asset resolved to ${String(loadedPath)} (platform=${process.platform}-${process.arch} OPENTUI_LIBC=${process.env.OPENTUI_LIBC ?? "unset"})`);
  }
